...
/* This function creates a 11 * 11 coordinate system that shows the locations of the enemy, and our base.   */
/* The position of the enemy is random and generates by the program. its displacement and its distance from */
/* our position are calculated.                                                                             */
void track_machine() {
    int x_our = 6;
    int y_our = 6;
    int x_enemy = 1;
...
/* This function generates two random numbers from 0 up to 11 for the coordinates of the enemy's base.  */
...
/* In this function we convert to characters to integers due to he given format, and print the          */
/* resulting integers to another file.                                                                  */
void encrypt_messages(char *file_path) {
    ...
    while((number = fgetc(fp)) != EOF) {
        /* Every current character will be added to the sum, and the count will be increased by the     */
        /* number of the added caracers.                                                                */
        sum += foo(number);
        count++;
        ...
        /* If the count is 3 and the current character is not '\n', sum will be printed to the output file. */
        /* Move cursor backwards by two character.                                                          */
        if(count == 3 && number != '\n') {
        ...
        /* If the current is '\n', a new line character will be printed and all the variables will be changed into 0. */
        else if(number == '\n') {
            fprintf(fout, "\n");
            line_count = 0;
            sum = 0;
...
/* This function, convert the characters that have been read in the above function due to the given format. */
int foo(char c) {
    int a;
    switch(c) {
        case ' ':  a = 0; break;
